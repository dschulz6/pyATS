...
This is a testing script intended to walk users through basic Cisco IOS
...
Testing:
    ...
    - interface count verification
        - execute `show version` command: 
                    basic command execution and data parsing; extract ethernet
                    interface counts; logs interface counts.
        - execute `show ip interface brief` command: 
                    basic command execution and data parsing; extract all
                    ethernet interfaces; logs number of interface counts.
        - verify ethernet interface counts from above commands.
...
This script was adapted from Wei Chen <weiche3@cisco.com> 'Copyright 2017, Cisco Systems'
dated 'Nov 15, 2017'  

Many Thanks and credits go to Mr. Chen!!!!
..........................................

This scripted was edited Jan 2020 DCSchulz to include the following:

class VerifyInterfaceCountTestcase
    def extract_interface_count

        modified to add embedded "if (match) -> else" decision making to extract interface
        count for FastEthernet interfaces if no Gigabit Ethernet interfaces are detected

    def verify_interface_count

        modified to add embedded "if (match) -> else" decision making to verify interface
        count for FastEthernet interfaces if no Gigabit Ethernet interfaces are detected
...
# 
#
__author__ = 'Dale C Schulz'
__copyright__ = 'None'
__email__ = '<daleschulz6@gmail.com>'
__date__= 'Jan 10, 2020'
    ...
    def extract_interface_count(self, device):
        ...
        try:
            ...
            if match: 
                ...
                logger.info(banner('\'show version\' returns {} GigE interfaces'
                                                .format(
                                                ethernet_intf_count
                ...
                match = re.search(r'(?P<ethernet>\d+) FastEthernet interfaces\r\n', result)    
                ethernet_intf_count = int(match.group('ethernet'))
                # log the interface counts
                logger.info(banner('\'show version\' returns {} FastEthernet interfaces'
                                            .format(
                                            ethernet_intf_count
